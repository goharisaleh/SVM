class SVM():
    def __init__(self, max_passes=100, kernel_type='linear', C=1.0, tolerance=0.001, sigma=1):
        self.kernels = {
...
#                     print(j)
    ...
    def accuracy(self, y_true, y_pred, X):
        correct_counter = 0
        y_pred = [self.predict(x) for x in X]
        for yt, yp in zip(y_train, y_pred):
            if yt == yp:
              correct_counter += 1
        return correct_counter / len(y_true)*100
